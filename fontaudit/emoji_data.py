"""Extended_Pictographic 码点表（源自 Unicode emoji-data.txt，15.x）。

用于 UAX #29 的 GB11 规则（emoji ZWJ 序列）与 ZWJ 断裂检测。
注：1F300..1FAFF 取整块（超集），未分配码点不会出现在真实文本中，无副作用。
"""
...
_STARTS = [a for a, _ in EXTENDED_PICTOGRAPHIC_RANGES]


def is_extended_pictographic(cp: int) -> bool:
    i = bisect_right(_STARTS, cp) - 1
    return i >= 0 and EXTENDED_PICTOGRAPHIC_RANGES[i][0] <= cp <= EXTENDED_PICTOGRAPHIC_RANGES[i][1]
